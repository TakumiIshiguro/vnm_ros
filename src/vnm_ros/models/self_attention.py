...
"""Transformer decoder adapted from visualnav-transformer under the MIT License.
...
class MultiLayerDecoder(nn.Module):
    def __init__(
        ...
        for index in range(len(output_layers) - 1):
            self.output_layers.append(
                nn.Linear(output_layers[index], output_layers[index + 1])
            )
