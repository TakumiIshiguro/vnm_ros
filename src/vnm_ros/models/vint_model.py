...
"""ViNT model adapted from visualnav-transformer under the MIT License.
    ...
    def __init__(
        ...
        if obs_encoder.split("-")[0] != "efficientnet":
            raise NotImplementedError(
                f"Unsupported ViNT observation encoder: {obs_encoder}"
            )

        self.obs_encoder = EfficientNet.from_name(obs_encoder, in_channels=3)
        self.num_obs_features = self.obs_encoder._fc.in_features
        goal_channels = 3 if self.late_fusion else 6
        self.goal_encoder = EfficientNet.from_name(
            "efficientnet-b0", in_channels=goal_channels
        )
