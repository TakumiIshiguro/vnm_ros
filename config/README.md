...
| `model_type` | 構築するモデル形式です。`vint`、`direction_vint`、または `nomad` を指定します。 |
...
目標画像の代わりに `/cmd_dir_intersection` の `cmd_dir` を入力するモデルです。
`cmd_dir` は `scenario_navigation_msgs/cmd_dir_intersection` の `int8[3]`
で、`[1, 0, 0]` が直進、`[0, 1, 0]` が左折、`[0, 0, 1]` が右折です。

| パラメータ | 意味 |
| --- | --- |
| `checkpoint_path` | DirectionViNTで読み込むモデル重みのパスです。 |
| `obs_encoding_size` | 画像特徴ベクトルと方向トークンの次元数です。 |
| `cmd_dir_dim` | `cmd_dir` の次元数です。通常は `3` です。 |
| `cmd_hidden_size` | `cmd_dir` を特徴ベクトルへ変換するMLPの中間次元です。 |
| `context_size` | 現在画像より前に使う画像枚数です。 |
| `image_size` | モデル入力画像の `[幅, 高さ]` です。 |
| `len_traj_pred` | モデルが予測する将来Waypoint数です。 |
| `learn_angle` | `true` の場合、WaypointのXYに加えて向きのcos/sinも学習・出力します。 |

DirectionViNTではTransformerへ `[過去画像..., 現在画像, 方向トークン]`
を入力します。既存のViNT/NoMaD checkpointとは互換ではないため、
`cmd_dir` 付きdatasetで再学習した重みが必要です。
...
| `cmd_dir_topic` | `direction_vint` で使う目標方向トピックです。`scenario_navigation_msgs/cmd_dir_intersection` の `cmd_dir` を読みます。 |
...
| `pretrained_load` | 事前学習重みの読み込み範囲です。`full` は全体、`obs_encoder` は観測画像Encoderのみを読み込みます。 |
...
Dataset作成と学習は別々に実行します。各launchは設定値を `config/*.yaml`
から読みます。

```bash
roslaunch vnm_ros create_dataset.launch
roslaunch vnm_ros train.launch
```

`direction_vint` を学習する場合、Dataset作成時に `topics.cmd_dir_topic`
の `cmd_dir` も `traj_data.pkl` に保存されます。学習時は `model_type` に応じて
Datasetが切り替わり、`direction_vint` では goal画像ではなく `cmd_dir`
をモデル入力に使います。
